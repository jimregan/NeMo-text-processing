...
class TimeFst(GraphFst):
    """Classifies Kashubian numeric hours and minutes."""
    ...
    def __init__(self, cardinal: GraphFst, ordinal: GraphFst, deterministic: bool = True):
        ...
        governed = time_graph("f_sg_loc", pynini.accep("ò") + delete_space + pynutil.insert(" "))
        hour_abbreviation = (
            pynini.accep("ò")
            + delete_space
            + pynutil.insert(" ")
            + pynini.cross("gòdz.", "gòdzënie")
            + delete_space
            + pynutil.insert(" ")
        )
        ...
        if not deterministic:
            alternatives = load_labels(get_abs_path("data/time/alternatives.tsv"))
