...
SCALE_NAMES = ["tësąc", "milión", "miliard", "bilión", "biliard", "trilión", "triliard"]
...
def _base_number_graphs(path: str) -> Dict[str, 'pynini.FstLike']:
    graphs = {}
    for word, number in load_labels(get_abs_path(path)):
        graph = pynini.cross(number, word)
        graphs[number] = graph if number not in graphs else graphs[number] | graph
    return {number: graph.optimize() for number, graph in graphs.items()}
...
def _noun_forms(lemma: str) -> Dict[str, str]:
    if lemma == "tësąc":
        return {key: value for key, value in load_labels(get_abs_path("data/numbers/tesac.tsv"))}

    oblique_singular = lemma + ("ie" if lemma.endswith("ón") else "ze")
    return {
        "sg_nom": lemma,
        "sg_gen": lemma + "a",
        "sg_dat": lemma + "owi",
        "sg_acc": lemma,
        "sg_ins": lemma + "ã",
        "sg_loc": oblique_singular,
        "sg_voc": oblique_singular,
        "pl_nom": lemma + "ë",
        "pl_gen": lemma + "ów",
        "pl_dat": lemma + "óm",
        "pl_acc": lemma + "ë",
...
class CardinalFst(GraphFst):
    """Classifies Kashubian cardinal numbers and exposes each inflectional graph in ``graphs``."""

    def __init__(self, deterministic: bool = True):
        super().__init__(name="cardinal", kind="classify", deterministic=deterministic)

        # TSV order sets the deterministic default; later forms are retained in
        # non-deterministic mode. Sources disagree on the dative/instrumental of
        # three and four, so both short and long forms are present in the data.
        digit_forms = get_digit_forms("data/numbers/digit_forms.tsv")
        digit_graphs = _forms_to_graphs(digit_forms, deterministic)
        base_digits = _base_number_graphs("data/numbers/digit.tsv")
        ...
        hundred_forms = dict(load_labels(get_abs_path("data/numbers/hundred_case.tsv")))
        ...
        for slot in sorted(ordinary_slots):
            ...
            if case == "compound":
                ...
                tens = tens_nom
                hundred = pynini.cross("1", hundred_forms[f"sg_{case}"])
                hundreds = (hundred | (NEMO_DIGIT - "1") @ hundreds_nom).optimize()
                component_join = join

            digit = self._digit_for_slot(digit_graphs, base_digits, slot)
            compound_digit = self._compound_digit_for_slot(digit_graphs, base_digits, slot)
            teen_for_slot = teen_compound if case == "compound" else teen
        ...
        self.compound = (self.graphs["compound"] + compound_boundary + pynini.closure(CSB_ALPHA, 1)).optimize()
    ...
    def _digit_for_slot(self, graphs, base_graphs, slot):
        choices = {
        ...
        selected = []
        for number, forms in graphs.items():
            selected.append(_select(forms, choices) if any(choice in forms for choice in choices) else base_graphs[number])
        return pynini.union(*selected).optimize()

    def _compound_digit_for_slot(self, graphs, base_graphs, slot):
