...
from nemo_text_processing.text_normalization.csb.graph_utils import all_to_graph
...
def complete_paradigm(partial: Dict[str, str], complete: bool = False):
    for target, source in load_labels(get_abs_path("data/ordinal/equivalences.tsv")):
        partial.setdefault(target, partial[source])
    if complete:
...
class OrdinalFst(GraphFst):
    """Classifies Kashubian ordinals and exposes adjective-inflected graphs in ``graphs``."""
